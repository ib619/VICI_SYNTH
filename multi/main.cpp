...
volatile int8_t played_octave [4];
volatile int8_t played_notes1[4] = {12, 12, 12, 12};
volatile int8_t played_octave1 [4];
uint32_t c = 0;
...
void decodeTask(void * pvParameters){
  ...
  while(1){
    ...
    played_octave[0] = RX_Message[0];
    played_notes[0] = RX_Message[1];
    played_octave[1] = RX_Message[2];
    played_notes[1] = RX_Message[3];
    played_octave[2] = RX_Message[4];
    played_notes[2] = RX_Message[5];
    played_octave[3] = RX_Message[6];
...
void displayUpdateTask(void * pvParameters){
  ...
  while(1){
    ...
    u8g2.print(played_octave1[0]);
    u8g2.setCursor(42,20);
    u8g2.print(played_octave1[1]);
    u8g2.setCursor(62,20);
    u8g2.print(played_octave1[2]);
    u8g2.setCursor(82,20);
    u8g2.print(played_octave1[3]);
    u8g2.setCursor(22,30);
    u8g2.print(oct_count[played_octave1[0]]);
    u8g2.setCursor(42,30);
    u8g2.print(oct_lenCount[played_octave1[0]]);
    u8g2.setCursor(62,30);
    u8g2.print(pressed_octave[2]);
    u8g2.setCursor(82,30);
    u8g2.print(pressed_octave[3]);
    u8g2.sendBuffer();
  }
}
...
void sampleISR(){
  ...
  int8_t index1 = 0;
    for (int8_t i = 0; i < 4; i++){
      if (pressed_keys[i] == 12){
        ...
        played_octave1[i] = played_octave[index];
        index++;
      } else {
        played_notes1[i] = pressed_keys[index1];
        played_octave1[i] = pressed_octave[index1];
        index1++;
      }
    }

  if(played_notes1[0] != 12){phaseAcc = sine_lut[sine_lut_start_index[played_notes1[0]]+lenCount];} else {phaseAcc = 0; counter = 0; lenCount = 0;}
  ...
  if(played_notes1[1] != 12){phaseAcc1 = sine_lut[sine_lut_start_index[played_notes1[1]]+lenCount1];} else {phaseAcc1 = 0; counter1 = 0; lenCount1 = 0;}
  ...
  if(played_notes1[2] != 12){phaseAcc2 = sine_lut[sine_lut_start_index[played_notes1[2]]+lenCount2];} else {phaseAcc2 = 0; counter2 = 0; lenCount2 = 0;}
  ...
  if(played_notes1[3] != 12){phaseAcc3 = sine_lut[sine_lut_start_index[played_notes1[3]]+lenCount3];} else {phaseAcc3 = 0; counter3 = 0; lenCount3 = 0;}
  ...
  int32_t Vout;
  if (is_main){Vout = phaseAccTotal >> 24;} else {Vout = 0;}
