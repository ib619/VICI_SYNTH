...
volatile int8_t pressed_keys[4];
const int16_t key_select [] = {256, 512, 1024, 2048, 16, 32, 64, 128, 1, 2, 4, 8};
const int16_t key_power [] = {8, 9, 10, 11, 4, 5, 6, 7, 0, 1, 2, 3};
volatile bool stop_play = true;
volatile bool stop_play1 = true;
volatile bool stop_play2 = true;
volatile bool stop_play3 = true;
...
void decodeMultipleKeys(const int8_t w0, const int8_t w1, const int8_t w2){
  ...
  for (int8_t i = 0; i < 12; i++){
    if(index > 3){break;}
    //Serial.println((value&key_select[i])>>key_power[i], BIN);
  ...
  stop_play = pressed_keys[0] == 12;
  stop_play1 = pressed_keys[1] == 12;
  stop_play2 = pressed_keys[2] == 12;
  stop_play3 = pressed_keys[3] == 12;
}
...
void displayUpdateTask(void * pvParameters){
  ...
  while(1){
    ...
    u8g2.setCursor(2,20);
    u8g2.print(Notes[currentIndex]);
    u8g2.setCursor(22,20);
    u8g2.print(knob0_obj.count/2);
    u8g2.setCursor(42,20);
    u8g2.print(knob1_obj.count/2);
    u8g2.setCursor(62,20);
    u8g2.print(knob2_obj.count/2);
    u8g2.setCursor(82,20);
    u8g2.print(knob3_obj.count/2);
    ///////////////
    u8g2.setCursor(22,30);
    u8g2.print(Notes[pressed_keys[0]]);
    u8g2.setCursor(42,30);
    u8g2.print(Notes[pressed_keys[1]]);
    u8g2.setCursor(62,30);
    u8g2.print(Notes[pressed_keys[2]]);
    u8g2.setCursor(82,30);
    u8g2.print(Notes[pressed_keys[3]]);
    u8g2.sendBuffer();
  }
}
...
void sampleISR(){
  ...
  if(!stop_play){phaseAcc = sine_lut[sine_lut_start_index[pressed_keys[0]]+lenCount];} else {phaseAcc = 0; counter = 0; lenCount = 0;}
  if(counter == 1){
    counter = 0;
    if(lenCount == sine_lut_sizes[pressed_keys[0]]){
    lenCount = 0;
    } else {
    lenCount++;
  }
  ...
  if(!stop_play1){phaseAcc1 = sine_lut[sine_lut_start_index[pressed_keys[1]]+lenCount1];} else {phaseAcc1 = 0; counter1 = 0; lenCount1 = 0;}
  if(counter1 == 1){
    counter1 = 0;
    if(lenCount1 == sine_lut_sizes[pressed_keys[1]]){
    lenCount1 = 0;
    } else {
    lenCount1++;
  }
  ...
  if(!stop_play2){phaseAcc2 = sine_lut[sine_lut_start_index[pressed_keys[2]]+lenCount2];} else {phaseAcc2 = 0; counter2 = 0; lenCount2 = 0;}
  if(counter2 == 1){
    counter2 = 0;
    if(lenCount2 == sine_lut_sizes[pressed_keys[2]]){
    lenCount2 = 0;
    } else {
    lenCount2++;
  }
  ...
  if(!stop_play3){phaseAcc3 = sine_lut[sine_lut_start_index[pressed_keys[3]]+lenCount3];} else {phaseAcc3 = 0; counter3 = 0; lenCount3 = 0;}
  if(counter3 == 1){
    counter3 = 0;
    if(lenCount3 == sine_lut_sizes[pressed_keys[3]]){
    lenCount3 = 0;
    } else {
    lenCount3++;
  }
