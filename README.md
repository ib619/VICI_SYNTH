...
<ol>
  ...
  <li>There is volume control with at least 8 increments operated by turning a knob</li>
...
The aformentioned structure allows minimal use of blocking dependencies for data protection, thus minimising the risk of deadlock.
...
The polyphony is achieved through 4 identical oscillators each playing a separate note. The outputs of the oscillators are pre-scaled according to the number of notes currently played, to avoid saturation and distortion. Then the samples are added together, achieving polyphony.
...
System 2 has many advantages as it reduces the priority of the sample computation code and samples do not need to be produced as exactly 22k Hz, they can be produced in bursts and other tasks can fit around this which allows for a more flexible programme. Using this system allows more flexibility in giving certain tasks more time and to lower worst case utilisation, consider average and not peak initiation interval for critical time analysis and even lower task priority.
...
A Knob class is used for all 4 knobs to update rotation values, set upper and lower limits and read current rotation value. This class is thread safe and behaves as a reenterant function which can be called from concurrent tasks without any synchronisation locks.
